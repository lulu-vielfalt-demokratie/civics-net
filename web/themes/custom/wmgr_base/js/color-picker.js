...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.wmgr_base.colorSchemes;
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-wmgr_base-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-wmgr_base-custom-color');
        });
    }
  }
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-wmgr_base-custom-color',
      textInput.getAttribute('data-wmgr_base-custom-color'),
    );
   ...
   */
  Drupal.behaviors.wmgr_baseColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'wmgr_base-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'wmgr_base-color-picker',
        '[data-drupal-selector="wmgr_base-color-picker"] input[type="text"]',
      );
